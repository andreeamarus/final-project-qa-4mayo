...
const postErrorCreateNoteSchema = require("../data/response/post-create-new-note-error-response-schema.json");
...
describe("Create new note test suite", () => {
  ...
  // hard coded var
  it("Hard coding create new note test", async () => {
    await spec()
      .post(`${baseUrl}/notes`)
      .withHeaders({
        "Content-Type": "application/json",
  ...
  it("Create new note - no authentication token - test", async () => {
    await spec()
      .post(`${baseUrl}/notes`)
      .withHeaders("Content-Type", "application/json")
      .withBody(requestBody)
